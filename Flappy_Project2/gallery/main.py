    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == QUIT or (event.type == KEYDOWN and event.key == K_ESCAPE):
                ...
                # print(FPSCLOCK.tick()) 
                FPSCLOCK.tick(FPS) 
                # print(playerx,playery,messagex,messagey)
...
def mainGame():
    ...
    while True: 
        ...
        if 0<upperPipes[0]['x']<5:
            ...
            # print("all pipes after appending")
            # print(upperPipes)
            # print(lowerPipes)
        
        #removing the pipe which pass the leftmost screen 
        if upperPipes[0]['x']< - GAME_SPRITES['pipe'][0].get_width():
            # print("all pipes before removing a pipes")
            # print(upperPipes)
            # print(lowerPipes)
            upperPipes.pop(0)
            lowerPipes.pop(0)
            # print("all pipes after removing a pipes")
            # print(upperPipes)
            # print(lowerPipes)
...
if __name__ == '__main__':
    ...
    #     GAME_SOUNDS[game_sound].play(-1)
    #     input()
    while True:
        welcomeScreen()
        mainGame()
    # print(GAME_SPRITES['pipe'][0].get_height())
    # offset = SCREENHEIGHT/3
    # print(int(SCREENHEIGHT - GAME_SPRITES['base'].get_height()-1.2*offset))
